    ...
    <% list.forEach(function(val){ %>
        <li>
            <h2 class="article-list-title text-overflow">
                <% if(user_name){ %>
                    <div class="article-list-title-tool">
                        <a href="<%=Url.article.edit(val.id)%>">编辑</a> | <a onclick="return confirm('确认删除吗？');" href="<%=Url.article.del(val.id)%>">删除</a>
                    </div>
                <% } %>
            ...
            <div class="article-list-more">
                <a href="<%=Url.article.view(val.id, val.url)%>">阅读全文 »</a>
            </div>
            <div class="article-list-tool clear">
                <ul class="fr">
                    ...
                    <li>
                        ...
                        <a href="<%=Url.article.list(val.list_data.id, val.list_data.url)%>">
                            <%=val.list_data.name%>
                        </a>
                    </li>
                    <% } %>
