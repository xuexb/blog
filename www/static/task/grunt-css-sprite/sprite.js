		...
		async.each(this.files, function(file, callback) {
			...
			async.waterfall([
				...
				function baseConfig(cb) {
					var 
					cssFilename,
					timeNow = grunt.template.today('HHmmss');


					//如果有自定义名
					if(options.imgName){
						cssFilename = options.imgName.indexOf(".png") > -1 ? options.imgName : options.imgName + ".png";
					} else {
					...
					sliceData.imgDest = fixPath(path.join(options.spritedest, cssFilename));
					sliceData.spriteImg = fixPath(path.join(options.spritepath, cssFilename)) + 
						sliceData.timestamp;
